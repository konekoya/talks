...
- 移動到下個方法 Step over next function call: 

- 進入到下一個方法裡 Step into next function call: 進到下一個方法裡。在我們這個例子裡，你可以在中斷點停在第 `29` 行後，再點選這個功能，這樣程式就會進入 `getNumber` 方法。

- 跳出目前的方法 Step out of current function: 跳出目前的方法，如果我們用前一個功能進入到一個方法裡，你想要跳出來，你就可以使用這個選項來離開目前所在的方法。
