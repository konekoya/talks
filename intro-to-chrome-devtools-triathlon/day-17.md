...
這就是我們新設置好的中斷點，現在我們一樣在 **Number 1** 跟 **Number 2** 這兩個輸入框裡輸入任意的數字，並按下 **Add Number1 and Number2** 這個按鈕。你的程式就會執行到這個中斷點，並停在 `31` 這裡。這樣是不是比剛剛這樣使用方便多了？

雖然使用 `debugger` 是沒有比直接在開發者工具裡設置中斷點快速，但是常常我們是需要使用 `debugger` 在我們的原始碼中，這有有幾個主要的原因：
...
- 我們設置在原始碼裡的 `debugger` 可以重覆執行，不會像直接設置在開發者工具裡的，每一次重新整理就會不見。而在除錯時，其實有蠻多情境是需要重新整理頁面的。
